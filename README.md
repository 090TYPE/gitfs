...
### CI монтирует по-настоящему

Оба контура заканчиваются живым томом на раннере GitHub, а не сборкой:
28/28 сценариев под Linux и 27/27 под Windows, каждый — **дважды по одному
тому**, плюс отдельным шагом главный инвариант «репозиторий не изменён».
Юнит-тесты проверяют всё ниже границы `IMountTarget`; всё, что стоило этому
проекту работоспособности, жило выше неё — в договоре с ядром.

Linux-контур вдобавок **заново снимает раскладку структур libfuse прямо на
раннере** и сверяет с фикстурой. Проверка написана на случай, когда другая
сборка libfuse сдвинет поле, — и уже подтвердила совпадение на образе,
отличном от того, где фикстура снималась.
...
| M3 | Адаптер WinFsp, `IMountTarget`, CLI | готово: 27/27 приёмки на томе в CI |
...
| последовательное чтение > 100 МБ/с | см. ниже — на этом репозитории не измеряется |
...
Пропускная способность здесь намеренно **не измеряется**, если в репозитории
нет блоба хотя бы на мегабайт. Цикл возвращается к началу файла, а чтение с
меньшего смещения заново открывает и разжимает поток блоба — на мелком файле
замер упирается именно в это. На репозитории самого gitfs, где крупнейший
файл 48 КБ, это давало 231 МБ/с на рабочей машине и 98 МБ/с на общем раннере
CI: разброс измерения, а не регрессия продукта. Бюджет, который нечем
измерить, честнее не измерять, чем объявить проваленным — настоящее число
даёт `tools/bench-fuse.sh` на 64-мегабайтном блобе.
...
SHA детерминированы. И **изолированы от окружения**: ни системный конфиг, ни
пользовательский, ни рефлог, ни фоновое обслуживание. Это не педантизм —
именно на этом CI падал пять прогонов подряд, каждый раз в другом тесте.

Симптомы выглядели как порча репозитория: git не мог прочитать блоб, который
сам только что записал; объектов на диске оказывалось двадцать вместо сотни;
`git add` получал ENOENT на временном файле. Диагноз дал единственный прогон,
где в фикстуре нашлось **три пакета там, где `repack -a -d` оставляет один**:
объекты не пропадали — их паковал посторонний. Фоновое обслуживание git.
`gc.auto = 0` его не останавливает: gc по умолчанию уходит в отдельный
процесс, а новые версии git идут к той же работе через `maintenance run
--auto`, и `GIT_CONFIG_NOSYSTEM` закрывает `/etc/gitconfig`, оставляя
`~/.gitconfig` — который на раннере есть и в который пишет сам
`actions/checkout`.

Отсюда правило, которому теперь следует фикстура: **зависеть только от того,
что настроила сама**. Проверено против намеренно враждебного глобального
конфига. Отдельные тесты-предохранители падают, если фикстура
