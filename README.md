...
`--keep-overlay`, `--portable-names`, `--history-limit`, `--cache-mb`,
`--max-blob-mb` — там же; `gitfs --help` перечисляет все.
...
трее, где состояние читается формой значка, а не только цветом. Один код на
Avalonia; монтируют Windows и Linux, на macOS кнопка заблокирована с
указанием причины.
...
412 тестов, одно и то же число на обеих платформах — и приёмка против
