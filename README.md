...
Честно о проверенности: **окно запускалось только на Windows**. Под Linux
приложение собирается и подключает адаптер FUSE, но графическую оболочку
там никто не открывал — под Linux проверен CLI, и проверен по-настоящему
(28 сценариев на смонтированном томе). На macOS кнопка монтирования
заблокирована с указанием причины: macFUSE ещё не подключён.
...
286 тестов плюс приёмка на живом томе — на обеих платформах:
...
* Цикл FUSE однопоточный: операции под Linux сериализованы. Бюджеты §16
  измерены только на Windows.
