...
public enum GitfsError
{
    ...
    NotSupported,
    TooLarge,
    /// <summary>Удаление директории. Надгробие песочницы прячет ровно один
    /// путь, поэтому «удалённая» директория оставляла бы своё содержимое
    /// читаемым и записываемым, но невидимым в листинге. Пока подкладка
    /// не умеет прятать поддерево, честнее отказать на первом же шаге,
    /// чем наполовину преуспеть.</summary>
    IsADirectory,
}
