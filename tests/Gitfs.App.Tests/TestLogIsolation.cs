...
internal static class TestLogIsolation
{
    ...
    internal static void Redirect()
    {
        ...
        // Оформление папок Проводника выключено по умолчанию: это забота
        // оболочки Windows, а не свойство дерева. Иначе каждая проверка
        // листинга в наборе несла бы desktop.ini и autorun.inf, и смысл её
        // размывался бы до неузнаваемости. Само оформление проверяется
        // отдельно — и на включённом, и на выключенном (ShellFoldersTests).
        Gitfs.Vfs.ShellFolders.Enabled = false;
    }
}
